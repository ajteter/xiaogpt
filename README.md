...
这个仓库当前不是原样上游，而是基于 `xiaogpt 3.23` 做了实际可运行、可部署、可继续产品化的整理版本。本文档内容截至 `2026-04-10`，以当前仓库代码和本地实测结果为准。
...
当前代码支持一个单独的“回捞上一句”口令，用来把最近一次未触发大模型的普通提问，再次转交给模型处理。
...
- 只有最近一次未触发大模型的普通提问会被缓存
- `开始持续对话`、`结束持续对话`、`更改提示词` 这类控制口令不会被当成可回捞问题
- 已经用 `keyword` 触发过大模型的问题不会进入回捞缓存
- 超过 `recall_previous_query_timeout` 后，再说回捞口令不会继续使用旧问题
- 如果当前没有可回捞的问题，音箱会提示“没有找到刚才的问题”
...
- `recall_previous_keyword` 可配置一个或多个“回捞上一句”口令，建议用短句，不建议和普通 `keyword` 复用
- `recall_previous_query_timeout` 默认 `90` 秒，单位是秒；设为 `0` 表示不限制回捞超时
...
- 优先用短句口令，不要只配一个过短的词
- 不要和 `keyword` 共用同一句口令
- 家庭语音场景建议把 `recall_previous_query_timeout` 设在 `30` 到 `90` 秒之间
